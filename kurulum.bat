@echo off
title Gojo Lang Kurulum Sihirbazi
color 0A

echo ==========================================
echo      GOJO LANG KURULUM SIHIRBAZI
echo ==========================================
echo.

REM --- 1. RUST KONTROLÜ (Gojo'nun kalbi Rust'tir) ---
echo [1/5] Rust derleyicisi kontrol ediliyor...
cargo --version >nul 2>&1
if %errorLevel% neq 0 (
    color 0C
    echo.
    REM --- DUZELTME: Parantezler kaldirildi ---
    echo [KRITIK HATA] Bilgisayarinizda Rust -Cargo- yuklu degil!
    echo Gojo, Rust altyapisini kullanir. Once Rust'i kurmalisiniz.
    echo.
...
copy /Y "%SOURCE_DIR%gojo.bat" "%TARGET_DIR%\" >nul

echo [4/5] Sistem yoluna (PATH) ekleniyor...
setx PATH "%PATH%;C:\Gojo" /M >nul
